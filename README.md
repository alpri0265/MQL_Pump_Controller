...
#### LCD 2004 4-бітний паралельний режим (альтернатива)
Якщо використовуєте LCD без I2C модуля, можна підключити в 4-бітному режимі:
...
- **VSS** → GND
- **VDD** → 5V
- **V0** → середній вивід потенціометра 10kΩ (для контрасту)
  - Крайні виводи потенціометра: один на 5V, інший на GND
- **A (Anode підсвітки)** → 5V через резистор 220Ω
- **K (Cathode підсвітки)** → GND
- **R/W** → GND (тільки запис)

**Примітка:** Для використання 4-бітного режиму потрібно змінити код:
- Замінити `LiquidCrystal_I2C` на `LiquidCrystal` в `ui.cpp`
- Змінити ініціалізацію: `LiquidCrystal lcd(RS, E, D4, D5, D6, D7)`
- Прибрати `Wire.begin()` та налаштування I2C
...
#### Для 4-бітного паралельного режиму:
Якщо використовуєте LCD без I2C модуля, потрібно:
1. Змінити `ui.cpp`: замінити `#include <LiquidCrystal_I2C.h>` на `#include <LiquidCrystal.h>`
2. Змінити ініціалізацію: `LiquidCrystal lcd(4, 5, 6, 7, 8, 9);` (RS, E, D4, D5, D6, D7)
3. Прибрати `Wire.begin()` з `uiBegin()`
4. Видалити або закоментувати рядок з `LCD_I2C_ADDR` в `config.h`
...
1. Відкрийте `MQL_Pump_Controller.ino`
...
- Перевірте живлення
- Використайте I2C сканер для визначення адреси

**Для 4-бітного режиму:**
- Перевірте підключення всіх пінів (RS, E, D4-D7)
- Налаштуйте контраст потенціометром на V0
- Перевірте підключення підсвітки (A/K)
- Перевірте, що R/W підключений до GND
- Перевірте живлення (VDD = 5V, VSS = GND)
...
### Структура проєкту:
- `MQL_Pump_Controller.ino` - основний скетч
